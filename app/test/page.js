"use client"

import React from 'react'

import styles from './divnext.module.css'

const Divnext = (props) => {
  return (
    <div className={styles['container']}>
      <div className={styles['divnext']}>
        <div className={styles['main']}>
          <div className={styles['div']}>
            <div className={styles['divcontainer']}>
              <div className={styles['divundefined']}>
                <div className={styles['divmx4']}>
                  <div className={styles['div01']}>
                    <div className={styles['divwfull']}>
                      <div
                        className={styles['p-v3descriptiontitle-big-iv2m-g']}
                      >
                        <span className={styles['text']}>
                          <span>
                            <span>We’re</span>
                            <br></br>
                            <span>changing</span>
                          </span>
                        </span>
                      </div>
                      <div
                        className={styles['p-v3descriptiontitle-big-iv2m-g1']}
                      >
                        <span className={styles['text005']}>
                          <span>the way you</span>
                        </span>
                      </div>
                    </div>
                    <button className={styles['link-button']}>
                      <div className={styles['divflex']}>
                        <div className={styles['div02']}>
                          <img
                            src="/play-store-transparent-icon.png.png"
                            alt="playstoretransparenticonpng118"
                            className={styles['playstoretransparenticonpng']}
                          />
                        </div>
                        <div className={styles['div03']}>
                          <img
                            src="/app-store-transparent-icon.png.png"
                            alt="appstoretransparenticonpng120"
                            className={styles['appstoretransparenticonpng']}
                          />
                        </div>
                        <div className={styles['bselfcenter']}>
                          <span className={styles['text007']}>
                            <span>Download the app</span>
                          </span>
                        </div>
                      </div>
                    </button>
                    <div className={styles['divwfull1']}>
                      <div
                        className={
                          styles['div-v3heroproblem-item-percentage4ee-kv']
                        }
                      ></div>
                    </div>
                  </div>
                  <div className={styles['divmxauto']}>
                    <img
                      src="/video-play-image.png.png"
                      alt="videoplayimagepng127"
                      className={styles['videoplayimagepng']}
                    />
                  </div>
                </div>
                {/* <img src alt="divhidden128" className={styles['divhidden']} /> */}
                <div className={styles['divhidden1']}>
                  <div className={styles['divflex01']}>
                    <div className={styles['div04']}>
                      <img
                        src="/liquide-broach.png.png"
                        alt="liquidebroachpng132"
                        className={styles['liquidebroachpng']}
                      />
                    </div>
                    <div className={styles['divml2margin']}>
                      <div className={styles['divml2']}>
                        <div className={styles['pbody1']}>
                          <span className={styles['text009']}>
                            <span>Liquide Trust</span>
                          </span>
                        </div>
                        <div className={styles['pcaption-white']}>
                          <span className={styles['text011']}>
                            <span>
                              <span>Trade setups by SEBI</span>
                              <br></br>
                              <span>Registered Research Analyst</span>
                            </span>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className={styles['divflex02']}>
                    <div className={styles['div05']}>
                      <img
                        src="/liquide-tick.png.png"
                        alt="liquidetickpng141"
                        className={styles['liquidetickpng']}
                      />
                    </div>
                    <div className={styles['divml2margin1']}>
                      <div className={styles['divml21']}>
                        <div className={styles['pbody101']}>
                          <span className={styles['text016']}>
                            <span>Liquide Secure</span>
                          </span>
                        </div>
                        <div className={styles['pcaption-white1']}>
                          <span className={styles['text018']}>
                            <span>
                              <span>Buy stocks 100% securely in</span>
                              <br></br>
                              <span>your own broker account</span>
                            </span>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className={styles['divflex03']}>
                    <div className={styles['div06']}>
                      <img
                        src="/dual-ring.png.png"
                        alt="dualringpng150"
                        className={styles['dualringpng']}
                      />
                    </div>
                    <div className={styles['divml2margin2']}>
                      <div className={styles['divml22']}>
                        <div className={styles['pbody102']}>
                          <span className={styles['text023']}>
                            <span>Transparent</span>
                          </span>
                        </div>
                        <div className={styles['pcaption-white2']}>
                          <span className={styles['text025']}>
                            <span>
                              <span>Realtime tracking &amp; full</span>
                              <br></br>
                              <span>user control</span>
                            </span>
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles['divmt12']}>
              <div className={styles['divcontainer1']}>
                <div className={styles['link']}>
                  <div className={styles['div07']}>
                    <img
                      src="/inc-42.png.png"
                      alt="inc42png161"
                      className={styles['inc42png']}
                    />
                  </div>
                </div>
                <div className={styles['link01']}>
                  <div className={styles['div08']}>
                    <img
                      src="/cnbc.png.png"
                      alt="cnbcpng164"
                      className={styles['cnbcpng']}
                    />
                  </div>
                </div>
                <div className={styles['link02']}>
                  <div className={styles['div09']}>
                    <img
                      src="/yourstory.png.png"
                      alt="yourstorypng167"
                      className={styles['yourstorypng']}
                    />
                  </div>
                </div>
                <div className={styles['link03']}>
                  <div className={styles['div10']}>
                    <img
                      src="/business-standard.png.png"
                      alt="businessstandardpng170"
                      className={styles['businessstandardpng']}
                    />
                  </div>
                </div>
                <div className={styles['link04']}>
                  <div className={styles['div11']}>
                    <img
                      src="/entrackr.png.png"
                      alt="entrackrpng173"
                      className={styles['entrackrpng']}
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles['div12']}>
            <div className={styles['div13']}>
              <div className={styles['div14']}>
                <div className={styles['divsticky']}>
                  <div className={styles['divflex04']}>
                    <div className={styles['pbody103']}>
                      <span className={styles['text030']}>
                        <span>OUR OFFERINGS</span>
                      </span>
                    </div>
                    <div className={styles['pdesktop-title']}>
                      <span className={styles['text032']}>
                        <span>How do we help you?</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['div15']}>
                    <div className={styles['divpx4']}>
                      <div className={styles['divflex05']}>
                        <div className={styles['div16']}>
                          <img
                            src="/tradeSetup.png.png"
                            alt="tradeSetuppng187"
                            className={styles['trade-setuppng']}
                          />
                        </div>
                        <div className={styles['div17']}>
                          <div className={styles['pdesktop-caption-bold']}>
                            <span className={styles['text034']}>
                              <span>Expert recommended trade setups</span>
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className={styles['divpx41']}>
                      <div className={styles['divflex06']}>
                        <div className={styles['div18']}>
                          <img
                            src="/limo.png.png"
                            alt="limopng194"
                            className={styles['limopng']}
                          />
                        </div>
                        <div className={styles['div19']}>
                          <div className={styles['pdesktop-caption-bold1']}>
                            <span className={styles['text036']}>
                              <span>LiMo: AI powered bot at your service</span>
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className={styles['divpx42']}>
                      <div className={styles['divflex07']}>
                        <div className={styles['div20']}>
                          <img
                            src="/portfoliohealth.png.png"
                            alt="portfoliohealthpng1101"
                            className={styles['portfoliohealthpng']}
                          />
                        </div>
                        <div className={styles['div21']}>
                          <div className={styles['pdesktop-caption-bold2']}>
                            <span className={styles['text038']}>
                              <span>Portfolio health checkup</span>
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className={styles['image']}>
                      <img
                        src="/image.png"
                        alt="image"
                        className={styles['image1']}
                      />
                    </div>
                    <div className={styles['divpx43']}>
                      <div className={styles['divflex08']}>
                        <div className={styles['div22']}>
                          <img
                            src="/expertHotline.png.png"
                            alt="expertHotlinepng1108"
                            className={styles['expert-hotlinepng']}
                          />
                        </div>
                        <div className={styles['div23']}>
                          <div className={styles['pdesktop-caption-bold3']}>
                            <span className={styles['text040']}>
                              <span>
                                Expert hotline support for your portfolio
                              </span>
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className={styles['divpx44']}>
                      <div className={styles['divflex09']}>
                        <img
                          src="/marketCommentary.png.png"
                          alt="marketCommentarypng1114"
                          className={styles['market-commentarypng']}
                        />
                        <div className={styles['div24']}>
                          <div className={styles['pdesktop-caption-bold4']}>
                            <span className={styles['text042']}>
                              <span>
                                <span>
                                  Market expedition with screeners &amp; market
                                </span>
                                <br></br>
                                <span>commentary</span>
                              </span>
                            </span>
                          </div>
                          <div className={styles['div25']}>
                            <div className={styles['pdesktop-caption3light']}>
                              <span className={styles['text047']}>
                                <span>
                                  Discover your next big investment opportunity
                                  quicker and easier.
                                </span>
                              </span>
                            </div>
                            <div className={styles['link05']}>
                              <div className={styles['divflex10']}>
                                <div
                                  className={
                                    styles['pdesktop-sub-header-medium']
                                  }
                                >
                                  <span className={styles['text049']}>
                                    <span>Explore market</span>
                                  </span>
                                </div>
                                <img
                                  src="/SVG1125-sc7a.svg"
                                  alt="SVG1125"
                                  className={styles['svg']}
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className={styles['divabsolute']}></div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles['divcontainer2']}>
            <div className={styles['div26']}>
              <div className={styles['link06']}>
                <img
                  src="/liquide-one-banner-xl.png.png"
                  alt="liquideonebannerxlpng1132"
                  className={styles['liquideonebannerxlpng']}
                />
              </div>
            </div>
            <div className={styles['divrelative']}>
              <div className={styles['div27']}>
                <div className={styles['divwfull2']}>
                  <div className={styles['pcaption5margin']}></div>
                  <div className={styles['p-v3descriptiontitlekjrdf']}>
                    <span className={styles['text051']}>
                      <span>We work with</span>
                    </span>
                  </div>
                  <div className={styles['p-v3descriptiontitlekjrdf1']}>
                    <span className={styles['text053']}>
                      <span>India&apos;s most trusted</span>
                    </span>
                  </div>
                  <div className={styles['p-v3descriptiontitlekjrdf2']}>
                    <span className={styles['text055']}>
                      <span>brokers</span>
                    </span>
                  </div>
                </div>
              </div>
              <img
                src="/trusted-brokers.png.png"
                alt="trustedbrokerspng1144"
                className={styles['trustedbrokerspng']}
              />
            </div>
          </div>
          <div className={styles['divmt121']}>
            <div className={styles['divundefined1']}>
              <div className={styles['divpb12']}>
                <div className={styles['div28']}>
                  <div className={styles['div29']}>
                    <div className={styles['divpx8']}>
                      <div className={styles['pcaption5']}></div>
                      <div className={styles['pheader-partner-regular']}>
                        <span className={styles['text057']}>
                          <span>
                            <span>Our target</span>
                            <br></br>
                            <span>accuracy is market</span>
                            <br></br>
                            <span>beating!</span>
                          </span>
                        </span>
                      </div>
                    </div>
                    <div className={styles['divmxautomargin']}>
                      <div className={styles['divmxauto01']}>
                        <div className={styles['divflex11']}>
                          {/* <img
                            src
                            alt="divhidden1158"
                            className={styles['divhidden2']}
                          /> */}
                          <div className={styles['divhiddenmargin']}>
                            <div className={styles['divhidden3']}>
                              <button className={styles['button']}>
                                <div className={styles['div30']}>
                                  {/* <img
                                    src
                                    alt="whiteleftarrowpng1163"
                                    className={styles['whiteleftarrowpng']}
                                  /> */}
                                </div>
                              </button>
                              <button className={styles['button01']}>
                                <div className={styles['div31']}>
                                  {/* <img
                                    src
                                    alt="whiterightarrowpng1166"
                                    className={styles['whiterightarrowpng']}
                                  /> */}
                                </div>
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className={styles['divwfull3']}>
                    <div className={styles['divscj-smf-ei']}>
                      <div className={styles['divsce-cydq-j']}>
                        <div className={styles['divscdkz-dqf']}>
                          <div className={styles['divsch-kmt-zm']}>
                            <div className={styles['divrec']}>
                              <div className={styles['divrec1']}>
                                <div className={styles['divscgsn-tzi']}>
                                  <div className={styles['divjustifystart']}>
                                    <div className={styles['divflex12']}>
                                      <div className={styles['divflex13']}>
                                        <div className={styles['div32']}>
                                          <img
                                            src="/liberty_Full.png.png"
                                            alt="libertyFullpng1179"
                                            className={
                                              styles['liberty-fullpng']
                                            }
                                          />
                                        </div>
                                      </div>
                                      <div className={styles['divgridmargin']}>
                                        <div className={styles['divgrid']}>
                                          <div className={styles['heading3']}>
                                            <span className={styles['text064']}>
                                              <span>LIBERTSHOE</span>
                                            </span>
                                          </div>
                                          <div
                                            className={
                                              styles['ptextleftmargin']
                                            }
                                          >
                                            <div
                                              className={styles['ptextleft']}
                                            >
                                              <span
                                                className={styles['text066']}
                                              >
                                                <span>Liberty Shoe...</span>
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className={styles['divflexmargin']}>
                                        <div className={styles['divflex14']}>
                                          <div
                                            className={styles['divmr1margin']}
                                          >
                                            <div className={styles['divmr1']}>
                                              <img
                                                src="/SVG1191-vdzck.svg"
                                                alt="SVG1191"
                                                className={styles['svg01']}
                                              />
                                            </div>
                                          </div>
                                          <div className={styles['divflex15']}>
                                            <div
                                              className={
                                                styles[
                                                  'pdesktop-sub-header-bold'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text068']}
                                              >
                                                <span>8.3</span>
                                              </span>
                                            </div>
                                            <div
                                              className={
                                                styles[
                                                  'psub-header-semiboldmargin'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text070']}
                                              >
                                                %
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin01']}
                                      >
                                        <div className={styles['divflex16']}>
                                          <div className={styles['divflex17']}>
                                            <div
                                              className={styles['divml1margin']}
                                            >
                                              <div className={styles['divml1']}>
                                                <img
                                                  src="/SVG1203-k9k9.svg"
                                                  alt="SVG1203"
                                                  className={styles['svg02']}
                                                />
                                              </div>
                                            </div>
                                            <div
                                              className={styles['divmxauto02']}
                                            >
                                              <div>
                                                <span
                                                  className={styles['text071']}
                                                >
                                                  <span>Target Hit</span>
                                                </span>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className={styles['pcaption21']}>
                                        <span className={styles['text073']}>
                                          <span>3 Oct &apos;23, 14:17 pm</span>
                                        </span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div className={styles['divrec2']}>
                                <div className={styles['divscgsn-tzi1']}>
                                  <div className={styles['divjustifystart1']}>
                                    <div className={styles['divflex18']}>
                                      <div className={styles['divflex19']}>
                                        <div className={styles['div33']}>
                                          <img
                                            src="/MFSL_CROP.png.png"
                                            alt="MFSLCROPpng1216"
                                            className={styles['mfslcro-ppng']}
                                          />
                                        </div>
                                      </div>
                                      <div className={styles['divgridmargin1']}>
                                        <div className={styles['divgrid1']}>
                                          <div className={styles['heading31']}>
                                            <span className={styles['text075']}>
                                              <span>MFSL</span>
                                            </span>
                                          </div>
                                          <div
                                            className={
                                              styles['ptextleftmargin1']
                                            }
                                          >
                                            <div
                                              className={styles['ptextleft1']}
                                            >
                                              <span
                                                className={styles['text077']}
                                              >
                                                <span>Max Financia...</span>
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin02']}
                                      >
                                        <div className={styles['divflex20']}>
                                          <div
                                            className={styles['divmr1margin1']}
                                          >
                                            <div className={styles['divmr11']}>
                                              <img
                                                src="/SVG1228-ho9l.svg"
                                                alt="SVG1228"
                                                className={styles['svg03']}
                                              />
                                            </div>
                                          </div>
                                          <div className={styles['divflex21']}>
                                            <div
                                              className={
                                                styles[
                                                  'pdesktop-sub-header-bold1'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text079']}
                                              >
                                                <span>7.3</span>
                                              </span>
                                            </div>
                                            <div
                                              className={
                                                styles[
                                                  'psub-header-semiboldmargin1'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text081']}
                                              >
                                                %
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin03']}
                                      >
                                        <div className={styles['divflex22']}>
                                          <div className={styles['divflex23']}>
                                            <div
                                              className={
                                                styles['divml1margin1']
                                              }
                                            >
                                              <div
                                                className={styles['divml11']}
                                              >
                                                <img
                                                  src="/SVG1240-o8u.svg"
                                                  alt="SVG1240"
                                                  className={styles['svg04']}
                                                />
                                              </div>
                                            </div>
                                            <div
                                              className={styles['divmxauto03']}
                                            >
                                              <div
                                                className={styles['pcaption22']}
                                              >
                                                <span
                                                  className={styles['text082']}
                                                >
                                                  <span>Target Hit</span>
                                                </span>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className={styles['pcaption23']}>
                                        <span className={styles['text084']}>
                                          <span>22 Aug &apos;23, 11:28 am</span>
                                        </span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div className={styles['divrec3']}>
                                <div className={styles['divscgsn-tzi2']}>
                                  <div className={styles['divjustifystart2']}>
                                    <div className={styles['divflex24']}>
                                      <div className={styles['divflex25']}>
                                        <div className={styles['div34']}>
                                          <img
                                            src="/LEMONTREE_CROP.png.png"
                                            alt="LEMONTREECROPpng1253"
                                            className={
                                              styles['lemontreecro-ppng']
                                            }
                                          />
                                        </div>
                                      </div>
                                      <div className={styles['divgridmargin2']}>
                                        <div className={styles['divgrid2']}>
                                          <div className={styles['heading32']}>
                                            <span className={styles['text086']}>
                                              <span>LEMONTREE</span>
                                            </span>
                                          </div>
                                          <div
                                            className={
                                              styles['ptextleftmargin2']
                                            }
                                          >
                                            <div
                                              className={styles['ptextleft2']}
                                            >
                                              <span
                                                className={styles['text088']}
                                              >
                                                <span>Lemon Tree H...</span>
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin04']}
                                      >
                                        <div className={styles['divflex26']}>
                                          <div
                                            className={styles['divmr1margin2']}
                                          >
                                            <div className={styles['divmr12']}>
                                              <img
                                                src="/SVG1265-rnye.svg"
                                                alt="SVG1265"
                                                className={styles['svg05']}
                                              />
                                            </div>
                                          </div>
                                          <div className={styles['divflex27']}>
                                            <div
                                              className={
                                                styles[
                                                  'pdesktop-sub-header-bold2'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text090']}
                                              >
                                                <span>7.3</span>
                                              </span>
                                            </div>
                                            <div
                                              className={
                                                styles[
                                                  'psub-header-semiboldmargin2'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text092']}
                                              >
                                                %
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin05']}
                                      >
                                        <div className={styles['divflex28']}>
                                          <div className={styles['divflex29']}>
                                            <div
                                              className={
                                                styles['divml1margin2']
                                              }
                                            >
                                              <div
                                                className={styles['divml12']}
                                              >
                                                <img
                                                  src="/SVG1277-jzeh.svg"
                                                  alt="SVG1277"
                                                  className={styles['svg06']}
                                                />
                                              </div>
                                            </div>
                                            <div
                                              className={styles['divmxauto04']}
                                            >
                                              <div
                                                className={styles['pcaption24']}
                                              >
                                                <span
                                                  className={styles['text093']}
                                                >
                                                  <span>Target Hit</span>
                                                </span>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className={styles['pcaption25']}>
                                        <span className={styles['text095']}>
                                          <span>18 Aug &apos;23, 14:31 pm</span>
                                        </span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              <div className={styles['divrec4']}>
                                <div className={styles['divscgsn-tzi3']}>
                                  <div className={styles['divjustifystart3']}>
                                    <div className={styles['divflex30']}>
                                      <div className={styles['divflex31']}>
                                        <div className={styles['div35']}>
                                          <img
                                            src="/PRAJIND_CROP.png.png"
                                            alt="PRAJINDCROPpng1290"
                                            className={
                                              styles['prajindcro-ppng']
                                            }
                                          />
                                        </div>
                                      </div>
                                      <div className={styles['divgridmargin3']}>
                                        <div className={styles['divgrid3']}>
                                          <div className={styles['heading33']}>
                                            <span className={styles['text097']}>
                                              <span>PRAJIND</span>
                                            </span>
                                          </div>
                                          <div
                                            className={
                                              styles['ptextleftmargin3']
                                            }
                                          >
                                            <div
                                              className={styles['ptextleft3']}
                                            >
                                              <span
                                                className={styles['text099']}
                                              >
                                                <span>Praj Industr...</span>
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin06']}
                                      >
                                        <div className={styles['divflex32']}>
                                          <div
                                            className={styles['divmr1margin3']}
                                          >
                                            <div className={styles['divmr13']}>
                                              <img
                                                src="/SVG1302-ongc.svg"
                                                alt="SVG1302"
                                                className={styles['svg07']}
                                              />
                                            </div>
                                          </div>
                                          <div className={styles['divflex33']}>
                                            <div
                                              className={
                                                styles[
                                                  'pdesktop-sub-header-bold3'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text101']}
                                              >
                                                <span>7.6</span>
                                              </span>
                                            </div>
                                            <div
                                              className={
                                                styles[
                                                  'psub-header-semiboldmargin3'
                                                ]
                                              }
                                            >
                                              <span
                                                className={styles['text103']}
                                              >
                                                %
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div
                                        className={styles['divflexmargin07']}
                                      >
                                        <div className={styles['divflex34']}>
                                          <div className={styles['divflex35']}>
                                            <div
                                              className={
                                                styles['divml1margin3']
                                              }
                                            >
                                              <div
                                                className={styles['divml13']}
                                              >
                                                <img
                                                  src="/SVG1314-7hmn.svg"
                                                  alt="SVG1314"
                                                  className={styles['svg08']}
                                                />
                                              </div>
                                            </div>
                                            <div
                                              className={styles['divmxauto05']}
                                            >
                                              <div
                                                className={styles['pcaption26']}
                                              >
                                                <span
                                                  className={styles['text104']}
                                                >
                                                  <span>Target Hit</span>
                                                </span>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                      <div className={styles['pcaption27']}>
                                        <span className={styles['text106']}>
                                          <span>17 Aug &apos;23, 12:40 pm</span>
                                        </span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className={styles['divsci-bkjdsmargin']}>
                        <div className={styles['divsci-bkjds']}>
                          {/* <button className={styles['buttonmargin']}>
                            <img
                              src
                              alt="Button1324"
                              className={styles['button02']}
                            />
                          </button> */}
                          <button className={styles['buttonmargin1']}>
                            <button className={styles['button03']}></button>
                          </button>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles['divundefined2']}>
            <div className={styles['div36']}>
              <div className={styles['div37']}>
                <div className={styles['divsticky1']}>
                  <div className={styles['pbody104']}>
                    <span className={styles['text108']}>
                      <span>THE QUESTION</span>
                    </span>
                  </div>
                  <div className={styles['pdesktop-title1']}>
                    <span className={styles['text110']}>
                      <span>
                        <span>Why choose</span>
                        <br></br>
                        <span>Liquide?</span>
                      </span>
                    </span>
                  </div>
                </div>
                <div className={styles['divrelative1']}>
                  <div className={styles['div38']}>
                    <img
                      src="/sebi.png.png"
                      alt="sebipng1337"
                      className={styles['sebipng']}
                    />
                    <img
                      src="/unbiasedImpartial.png.png"
                      alt="unbiasedImpartialpng1338"
                      className={styles['unbiased-impartialpng']}
                    />
                  </div>
                  <div className={styles['div39']}>
                    <img
                      src="/aiEngine.png.png"
                      alt="aiEnginepng1340"
                      className={styles['ai-enginepng']}
                    />
                  </div>
                  <div className={styles['divmt48margin']}>
                    <div className={styles['divmt48']}>
                      <img
                        src="/alumni.png.png"
                        alt="alumnipng1343"
                        className={styles['alumnipng']}
                      />
                      <img
                        src="/tradeHits.png.png"
                        alt="tradeHitspng1344"
                        className={styles['trade-hitspng']}
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles['divundefined3']}>
            <div className={styles['div40']}>
              <div className={styles['divmt122']}>
                <div className={styles['divflex36']}>
                  <div className={styles['pbody105']}></div>
                  <div className={styles['pdesktop-title2']}>
                    <span className={styles['text115']}>
                      <span>We take pride in our numbers!</span>
                    </span>
                  </div>
                </div>
                <div className={styles['div41']}>
                  <img
                    src="/updatedPrideNumber.png.png"
                    alt="updatedPrideNumberpng1354"
                    className={styles['updated-pride-numberpng']}
                  />
                </div>
              </div>
            </div>
          </div>
          <div className={styles['divcontainer3']}>
            <div className={styles['divmx41']}>
              <div className={styles['div42']}>
                <div className={styles['divwfull4']}>
                  <div className={styles['pcaption5margin1']}></div>
                  <div className={styles['p-v3descriptiontitlekjrdf3']}>
                    <span className={styles['text117']}>
                      <span>People love what</span>
                    </span>
                  </div>
                  <div className={styles['p-v3descriptiontitlekjrdf4']}>
                    <span className={styles['text119']}>
                      <span>we&apos;re doing!</span>
                    </span>
                  </div>
                </div>
                <div className={styles['divmxautomargin1']}>
                  <div className={styles['divmxauto06']}>
                    <div className={styles['divflex37']}>
                      {/* <img
                        src
                        alt="divhidden1368"
                        className={styles['divhidden4']}
                      /> */}
                      <div className={styles['divhiddenmargin1']}>
                        <div className={styles['divhidden5']}>
                          <button className={styles['button04']}>
                            <div className={styles['div43']}>
                              {/* <img
                                src
                                alt="whiteleftarrowpng1373"
                                className={styles['whiteleftarrowpng1']}
                              /> */}
                            </div>
                          </button>
                          <button className={styles['button05']}>
                            <div className={styles['div44']}>
                              {/* <img
                                src
                                alt="whiterightarrowpng1376"
                                className={styles['whiterightarrowpng1']}
                              /> */}
                            </div>
                          </button>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles['divscj-smf-ei1']}>
                <div className={styles['divsce-cydq-j1']}>
                  <div className={styles['divscdkz-dqf1']}>
                    <div className={styles['divsch-kmt-zm1']}>
                      <div className={styles['divrec5']}>
                        <div className={styles['divrec6']}>
                          <div className={styles['divscgsn-tzi4']}>
                            <div className={styles['divmy8']}>
                              <div className={styles['divwfull5']}>
                                <div className={styles['divabsolute1']}>
                                  <div className={styles['psubheader-bold']}>
                                    <span className={styles['text121']}>
                                      <span>
                                        Making investing part of your DNA
                                      </span>
                                    </span>
                                  </div>
                                </div>
                                <span className={styles['text123']}>
                                  <span>
                                    <span>
                                      Liquide’s accuracy is phenomenal. I trust
                                      the team’s
                                    </span>
                                    <br></br>
                                    <span>
                                      judgement fully and the automated alerts
                                      make life super
                                    </span>
                                    <br></br>
                                    <span>
                                      simple. They are set to go places!
                                    </span>
                                  </span>
                                </span>
                                <div className={styles['divmt5']}>
                                  <div className={styles['pbody106']}></div>
                                </div>
                                <div className={styles['divwmax']}>
                                  <div className={styles['divrelativemargin']}>
                                    <div className={styles['divrelative2']}>
                                      <div className={styles['div45']}>
                                        <img
                                          src
                                          alt="jayantvpng1400"
                                          className={styles['jayantvpng']}
                                        />
                                      </div>
                                      <img
                                        src
                                        alt="indianflagpng1401"
                                        className={styles['indianflagpng']}
                                      />
                                    </div>
                                  </div>
                                  <div className={styles['divflexcol']}>
                                    <div className={styles['pbody107']}>
                                      <span className={styles['text130']}>
                                        <span>Jayanth V</span>
                                      </span>
                                    </div>
                                    <span className={styles['text132']}>
                                      <span>
                                        Senior Product Manager, Amazon Payment
                                        Products
                                      </span>
                                    </span>
                                  </div>
                                </div>
                                <img
                                  src="/testimonial-background.png.png"
                                  alt="testimonialbackgroundpng1402"
                                  className={styles['testimonialbackgroundpng']}
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className={styles['divrec7']}>
                          <div className={styles['divscgsn-tzi5']}>
                            <div className={styles['divmy81']}></div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className={styles['divsci-bkjdsmargin1']}>
                  <div className={styles['divsci-bkjds1']}>
                    <button className={styles['buttonmargin2']}>
                      <button className={styles['button06']}></button>
                    </button>
                    <button className={styles['buttonmargin3']}>
                      {/* <img
                        src
                        alt="Button1428"
                        className={styles['button07']}
                      /> */}
                    </button>
                    <button className={styles['buttonmargin4']}>
                      <button className={styles['button08']}></button>
                    </button>
                    <button className={styles['buttonmargin5']}>
                      <button className={styles['button09']}></button>
                    </button>
                    <button className={styles['buttonmargin6']}>
                      <button className={styles['button10']}></button>
                    </button>
                    <button className={styles['buttonmargin7']}>
                      <button className={styles['button11']}></button>
                    </button>
                    <button className={styles['buttonmargin8']}>
                      <button className={styles['button12']}></button>
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles['divmt28']}>
            <div className={styles['divcontainer4']}>
              <div className={styles['pcaption51']}></div>
              <div className={styles['divpx45']}>
                <div className={styles['div46']}>
                  <div className={styles['divborderb2']}>
                    <div className={styles['divflex38']}>
                      <div className={styles['pbody108']}>
                        <span className={styles['text134']}>
                          <span>How to start with Liquide?</span>
                        </span>
                      </div>
                      <div className={styles['divw6']}>
                        <img
                          src="/SVG1450-x4b5.svg"
                          alt="SVG1450"
                          className={styles['svg09']}
                        />
                      </div>
                    </div>
                    <div className={styles['divmt4']}>
                      <div className={styles['pbody2']}>
                        <span className={styles['text136']}>
                          <span>
                            <span>
                              Connect your existing brokerage account and get
                              set to use Liquide in under a minute. Four things
                              you can immediately do on
                            </span>
                            
                            <span> Liquide.</span>
                          </span>
                        </span>
                      </div>
                      <div className={styles['pmy2']}>
                        <span className={styles['text141']}>
                          <span className={styles['text142']}>
                            1.
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span className={styles['text143']}>
                            Browse Stock Picks:
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span>
                            <span>
                              Buy stocks curated by our specialist investment
                              team in the ‘Trades’ section. Liquide
                              automatically
                            </span>
                            <br></br>
                            <span>informs you when to sell.</span>
                          </span>
                        </span>
                      </div>
                      <div className={styles['pmy21']}>
                        <span className={styles['text148']}>
                          <span className={styles['text149']}>
                            2.
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span className={styles['text150']}>
                            Ask LiMo:
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span>
                            <span>
                              Access automated research reports through our AI
                              powered and expert supervised bot which gives a
                              clear
                            </span>
                            <br></br>
                            <span>buy/sell or hold recommendation.</span>
                          </span>
                        </span>
                      </div>
                      <div className={styles['pmy22']}>
                        <span className={styles['text155']}>
                          <span className={styles['text156']}>
                            3.
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span className={styles['text157']}>
                            Analyze Markets:
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span>
                            Easily research market movement and investor
                            sentiment; browse top rated stocks.
                          </span>
                        </span>
                      </div>
                      <div className={styles['pmy23']}>
                        <span className={styles['text159']}>
                          <span className={styles['text160']}>
                            4.
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span className={styles['text161']}>
                            Check Portfolio Health:
                            <span
                              dangerouslySetInnerHTML={{
                                __html: ' ',
                              }}
                            />
                          </span>
                          <span>
                            Track your portfolio health and manage your stock
                            holdings.
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className={styles['div47']}>
                  <div className={styles['divborderb21']}>
                    <div className={styles['divflex39']}>
                      <div className={styles['pbody109']}>
                        <span className={styles['text163']}>
                          <span>
                            Can I login via my current broker account?
                          </span>
                        </span>
                      </div>
                      <div className={styles['divw61']}>
                        <img
                          src="/SVG1469-rtfp.svg"
                          alt="SVG1469"
                          className={styles['svg10']}
                        />
                      </div>
                    </div>
                  </div>
                </div>
                <div className={styles['div48']}>
                  <div className={styles['divmxauto07']}>
                    <div className={styles['divflex40']}>
                      <div className={styles['pbody110']}>
                        <span className={styles['text165']}>
                          <span>
                            What is a stock pick and how do I buy them?
                          </span>
                        </span>
                      </div>
                      <div className={styles['divw62']}>
                        <img
                          src="/SVG1477-r4f.svg"
                          alt="SVG1477"
                          className={styles['svg11']}
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles['divwfull6']}>
                <div className={styles['link07']}>
                  <div className={styles['divflex41']}>
                    <div className={styles['pgradienttextbase']}>
                      <span className={styles['text-see']}>See All</span>
                    </div>
                    <img
                      src="/SVG1484-6p5.svg"
                      alt="SVG1484"
                      className={styles['svg12']}
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className={styles['divblurred-view']}>
          {/* <img src alt="pseudo1487" className={styles['pseudo']} />
          <img src alt="pseudo1488" className={styles['pseudo1']} /> */}
          <div className={styles['footer']}>
            <div className={styles['div49']}>
              <div className={styles['divflex42']}>
                <div className={styles['link08']}>
                  <div className={styles['div50']}>
                    <img
                      src="/Logo.png.png"
                      alt="Logopng1494"
                      className={styles['logopng']}
                    />
                  </div>
                </div>
                <div className={styles['divflex43']}>
                  <div className={styles['link09']}>
                    <div className={styles['div51']}>
                      <img
                        src="/share.png.png"
                        alt="sharepng1498"
                        className={styles['sharepng']}
                      />
                    </div>
                  </div>
                  <div className={styles['div52']}>
                    <div className={styles['link10']}>
                      <div className={styles['div53']}>
                        <img
                          src="/instagram-vector.png.png"
                          alt="instagramvectorpng1502"
                          className={styles['instagramvectorpng']}
                        />
                      </div>
                    </div>
                  </div>
                  <div className={styles['div54']}>
                    <div className={styles['link11']}>
                      <div className={styles['div55']}>
                        <img
                          src="/facebook-vector.png.png"
                          alt="facebookvectorpng1506"
                          className={styles['facebookvectorpng']}
                        />
                      </div>
                    </div>
                  </div>
                  <div className={styles['div56']}>
                    <div className={styles['link12']}>
                      <div className={styles['div57']}>
                        <img
                          src="/twitter-vector.png.png"
                          alt="twittervectorpng1510"
                          className={styles['twittervectorpng']}
                        />
                      </div>
                    </div>
                  </div>
                  <div className={styles['div58']}>
                    <div className={styles['link13']}>
                      <div className={styles['div59']}>
                        <img
                          src="/linkedin-vector.png.png"
                          alt="linkedinvectorpng1514"
                          className={styles['linkedinvectorpng']}
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles['divhidden6']}>
                <div className={styles['div60']}>
                  <div className={styles['ptextleft4']}>
                    <span className={styles['text167']}>
                      <span>Liquide</span>
                    </span>
                  </div>
                  <div className={styles['link14']}>
                    <div className={styles['psub-header3']}>
                      <span className={styles['text169']}>
                        <span>Home</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link15']}>
                    <div className={styles['psub-header301']}>
                      <span className={styles['text171']}>
                        <span>About us</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link16']}>
                    <div className={styles['psub-header302']}>
                      <span className={styles['text173']}>
                        <span>Trade</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link17']}>
                    <div className={styles['psub-header303']}>
                      <span className={styles['text175']}>
                        <span>Career</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link18']}>
                    <div className={styles['psub-header304']}>
                      <span className={styles['text177']}>
                        <span>Help &amp; support</span>
                      </span>
                    </div>
                  </div>
                </div>
                <div className={styles['div61']}>
                  <div className={styles['ptextleft5']}>
                    <span className={styles['text179']}>
                      <span>Products</span>
                    </span>
                  </div>
                  <div className={styles['link19']}>
                    <div className={styles['psub-header305']}>
                      <span className={styles['text181']}>
                        <span>Liquide app</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link20']}>
                    <div className={styles['psub-header306']}>
                      <span className={styles['text183']}>
                        <span>Portfolio checkup</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link21']}>
                    <div className={styles['psub-header307']}>
                      <span className={styles['text185']}>
                        <span>Liquide One</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link22']}>
                    <div className={styles['psub-header308']}>
                      <span className={styles['text187']}>
                        <span>Academy</span>
                      </span>
                    </div>
                  </div>
                </div>
                <div className={styles['div62']}>
                  <div className={styles['ptextleft6']}>
                    <span className={styles['text189']}>
                      <span>Resources</span>
                    </span>
                  </div>
                  <div className={styles['link23']}>
                    <div className={styles['psub-header309']}>
                      <span className={styles['text191']}>
                        <span>News that moves</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link24']}>
                    <div className={styles['psub-header310']}>
                      <span className={styles['text193']}>
                        <span>IPO Reports</span>
                      </span>
                    </div>
                  </div>
                </div>
                <div className={styles['div63']}>
                  <div className={styles['ptextleft7']}>
                    <span className={styles['text195']}>
                      <span>Policy</span>
                    </span>
                  </div>
                  <div className={styles['link25']}>
                    <div className={styles['psub-header311']}>
                      <span className={styles['text197']}>
                        <span>Privacy</span>
                      </span>
                    </div>
                  </div>
                  <div className={styles['link26']}>
                    <div className={styles['psub-header312']}>
                      <span className={styles['text199']}>
                        <span>Terms of use</span>
                      </span>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles['div64']}>
                <div className={styles['pmx4margin']}>
                  <span className={styles['text201']}>
                    Made with ❤️ in India
                  </span>
                </div>
                <div className={styles['divflex44']}>
                  <div className={styles['link27']}>
                    <div className={styles['div65']}>
                      <img
                        src="/play-store_.png.png"
                        alt="playstorepng1573"
                        className={styles['playstorepng']}
                      />
                    </div>
                  </div>
                  <div className={styles['link28']}>
                    <div className={styles['div66']}>
                      <img
                        src="/app-store_.png.png"
                        alt="appstorepng1576"
                        className={styles['appstorepng']}
                      />
                    </div>
                  </div>
                </div>
              </div>
              {/* <img src alt="divmxauto1577" className={styles['divmxauto08']} /> */}
            </div>
            <div className={styles['divmxauto09']}>
              <div className={styles['divtextgray300']}>
                <div className={styles['pcaption']}>
                  <span className={styles['text202']}>
                    <span>
                      <span>
                        Liquide Solutions Private Limited makes no warranties or
                        representations, express or implied, on products and
                        services offered through the
                      </span>
                      <br></br>
                      <span>
                        platform. It accepts no liability for any damages or
                        losses, however, caused in connection with the use of,
                        or on the reliance of its advisory or
                      </span>
                      <br></br>
                      <span>related services.</span>
                    </span>
                  </span>
                </div>
                <div className={styles['p']}></div>
                <div className={styles['pcaption1']}>
                  <span className={styles['text209']}>
                    <span>
                      <span>
                        Past performance is not indicative of future returns.
                        Please consider your specific investment requirements,
                        risk tolerance, goal, time frame, risk
                      </span>
                      <br></br>
                      <span>
                        and reward balance and the cost associated with the
                        investment before choosing a fund, or designing a
                        portfolio that suits your needs.
                      </span>
                      <br></br>
                      <span>
                        Performance and returns of any investment portfolio can
                        neither be predicted nor guaranteed.
                      </span>
                    </span>
                  </span>
                </div>
              </div>
            </div>
            <div className={styles['divwfull7']}>
              <div className={styles['divmxauto10']}>
                <div className={styles['divflexmargin08']}>
                  <div className={styles['divflex45']}>
                    <div className={styles['div67']}>
                      <img
                        src="/location.png.png"
                        alt="locationpng1590"
                        className={styles['locationpng']}
                      />
                    </div>
                    <div className={styles['divcol']}>
                      <div className={styles['pcaption-white3']}>
                        <span className={styles['text216']}>
                          <span>Signet Wing A, Cessna Business Park,</span>
                        </span>
                      </div>
                      <div className={styles['pcaption-white4']}>
                        <span className={styles['text218']}>
                          <span>Bengaluru, Karnataka 560103</span>
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className={styles['divflexmargin09']}>
                  <div className={styles['divflex46']}>
                    <div className={styles['div68']}>
                      <img
                        src="/Phone.png.png"
                        alt="Phonepng1599"
                        className={styles['phonepng']}
                      />
                    </div>
                    <div className={styles['divcol1']}>
                      <div
                        className={styles['pcaption2']}
                      >
                        <span className={styles['text220']}>
                          <span>Call us at:</span>
                        </span>
                      </div>
                      <span className={styles['text222']}>
                        <span>+91 700 643 0481</span>
                      </span>
                    </div>
                  </div>
                </div>
                <div className={styles['divflexmargin10']}>
                  <div className={styles['divflex47']}>
                    <div className={styles['div69']}>
                      <img
                        src="/email.png.png"
                        alt="emailpng1607"
                        className={styles['emailpng']}
                      />
                    </div>
                    <div className={styles['divcol2']}>
                      <div className={styles['pcaption3']}>
                        <span className={styles['text224']}>
                          <span>For assistance, write to us:</span>
                        </span>
                      </div>
                      <span className={styles['text226']}>
                        <span>support@liquide.life</span>
                      </span>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles['divmb24']}>
                <span className={styles['text228']}>
                  <span>SEBI Registration Details</span>
                </span>
                <div className={styles['divflex48']}>
                  <div className={styles['pmt6']}>
                    <span className={styles['text230']}>
                      <span>
                        Name: Liquide Solutions Private Limited | RA No:
                        INH000009816 | Reg. Type: Corporate | Validity:
                        Perpetual  
                      </span>
                    </span>
                  </div>
                </div>
                <div className={styles['ptextgray500']}>
                  <span className={styles['text232']}>
                    <span>
                      Associated SEBI regional office: SEBI, Jeevan Mangal
                      Building, Hayes Rd, off, Residency Rd, Shanthala Nagar,
                      Ashok Nagar, Bengaluru, Karnataka 560025
                    </span>
                  </span>
                </div>
                <div className={styles['ptextgray5001']}>
                  <span className={styles['text234']}>
                    <span>
                      For regulatory disclosures including the ‘Complaints
                      disclosure’ and the SEBI ‘Investor Charter’, 
                    </span>
                  </span>
                  <div className={styles['pflex']}>
                    <span className={styles['text236']}>
                      <span>please click</span>
                    </span>
                    <div className={styles['link29']}>
                      <div className={styles['pbreaknormal']}>
                        <span className={styles['text238']}>
                          <span>Here</span>
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className={styles['nav']}>
          <div className={styles['divcontainer5']}>
            <div className={styles['link30']}>
              <div className={styles['div70']}>
                <img
                  src="/Logo.png.png"
                  alt="Logopng1630"
                  className={styles['logopng1']}
                />
              </div>
            </div>
            <div className={styles['divhidden7']}>
              <div className={styles['link31']}>
                <img
                  src="/SVGmargin1633-js9l.svg"
                  alt="SVGmargin1633"
                  className={styles['sv-gmargin']}
                />
                <div className={styles['psub-partner-semi-bold']}>
                  <span className={styles['text240']}>
                    <span>Home</span>
                  </span>
                </div>
              </div>
              <div className={styles['linkmargin']}>
                <div className={styles['link32']}>
                  <img
                    src="/SVGmargin1641-7rw7.svg"
                    alt="SVGmargin1641"
                    className={styles['sv-gmargin1']}
                  />
                  <div className={styles['psub-partner-semi-bold1']}>
                    <span className={styles['text242']}>
                      <span>Trade</span>
                    </span>
                  </div>
                </div>
              </div>
              <div className={styles['linkmargin1']}>
                <div className={styles['link33']}>
                  <img
                    src="/SVGmargin1649-zwhj.svg"
                    alt="SVGmargin1649"
                    className={styles['sv-gmargin2']}
                  />
                  <div className={styles['psub-partner-semi-bold2']}>
                    <span className={styles['text244']}>
                      <span>News</span>
                    </span>
                  </div>
                </div>
              </div>
              <div className={styles['linkmargin2']}>
                <div className={styles['link34']}>
                  <img
                    src="/SVG1657-6o.svg"
                    alt="SVG1657"
                    className={styles['svg13']}
                  />
                  <div className={styles['psub-partner-semi-boldmargin']}>
                    <span className={styles['text246']}>
                      <span>Blog</span>
                    </span>
                  </div>
                </div>
              </div>
              <div className={styles['linkmargin3']}>
                <div className={styles['link35']}>
                  <div className={styles['div71']}>
                    <img
                      src="/academy-navbar-icon.png.png"
                      alt="academynavbariconpng1664"
                      className={styles['academynavbariconpng']}
                    />
                  </div>
                  <div className={styles['psub-partner-semi-bold3']}>
                    <span className={styles['text248']}>
                      <span>Academy</span>
                    </span>
                  </div>
                </div>
              </div>
              <div className={styles['link36']}>
                <button className={styles['button13']}>
                  <div className={styles['div72']}>
                    <img
                      src="/search-icon.png.png"
                      alt="searchiconpng1670"
                      className={styles['searchiconpng']}
                    />
                  </div>
                </button>
              </div>
            </div>
            <div className={styles['divhidden8']}>
              <div className={styles['link37']}>
                <div className={styles['div73']}>
                  <img
                    src="/barcode-icon.png.png"
                    alt="barcodeiconpng1674"
                    className={styles['barcodeiconpng']}
                  />
                </div>
                <div className={styles['pfontbold']}>
                  <span className={styles['text250']}>
                    <span>Download app</span>
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default Divnext
